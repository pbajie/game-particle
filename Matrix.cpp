...
enum MatrixRowEnum{MATRIX_ROW_0, MATRIX_ROW_1, MATRIX_ROW_2, MATRIX_ROW_3};
...
Matrix& Matrix::operator/=(float rhs)
{
	...
	__m128 res = _mm_set1_ps(1.0f / rhs);
	v0._m = _mm_mul_ps(res, v0._m);
	v1._m = _mm_mul_ps(res, v1._m);
	v2._m = _mm_mul_ps(res, v2._m);
	v3._m = _mm_mul_ps(res, v3._m);
	//m0 *= inv_rhs;
	//m1 *= inv_rhs;
	//m2 *= inv_rhs;
	//m3 *= inv_rhs;
...
void Matrix::setRotZMatrix(float az)
{
	...
	//	{	0		0		1		0	}
	//	{	0		0		0		1	}
	float a = sin(az), b = (float)cosf(az);
